...
using SSoft.PhoneBook.MultiTenancy.Dto;
using SSoft.PhoneBook.Notifications;
using Shouldly;
using Xunit;

namespace SSoft.PhoneBook.Tests.MultiTenancy
{
        ...
        public async Task Create_Update_And_Delete_Tenant_Test()
        {
            ...
            await _tenantAppService.CreateTenant(
                new CreateTenantInput
                {
                    ...
                    IsActive = true,
                    ConnectionString = "Server=localhost; Database=PhoneBookTest_" + Guid.NewGuid().ToString("N") + "; Trusted_Connection=True;"
                });
